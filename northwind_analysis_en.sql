...
-- Presented in tabular format and column chart (see charts/).
...
-- Presented in tabular format and line chart (see charts/).
...
-- Presented in tabular format and bar chart (see charts/).
SELECT strftime('%Y', o.OrderDate) AS Year,
       SUM(od.UnitPrice * od.Quantity * (1 - od.Discount)) AS TotalSales
FROM Orders o
JOIN 'Order Details' od ON o.OrderID = od.OrderID
...
-- United Package and Federal Shipping.
-- Presented in tabular format and pie chart (see charts/).
...
-- 39.
-- Show monthly revenue for 1997 comparing the 'Beverages' and
-- 'Confections' categories. Each row represents a month,
